...
class AcceptTripRepoImpl extends AcceptTripRepo {
  final ApiService _apiService;

  AcceptTripRepoImpl(this._apiService);

  @override
  Future<Either<Failur, Map<String, dynamic>>> acceptTrip({
    required String tripId,
  }) async {
    try {
      AppLogger.debug("---------------- [🚀 START ACCEPT TRIP REQUEST] ----------------");
      AppLogger.debug("📌 Trip ID: $tripId");
      AppLogger.debug("🔗 Endpoint: /trips/accept-trip/$tripId");

      var response = await _apiService.get(
        endpoint: "/trips/accept-trip/$tripId",
        needToken: true,
      );
      ...
      if (response.data["success"] == true) {
        AppLogger.debug("✅ Success: Trip accepted successfully by server.");
